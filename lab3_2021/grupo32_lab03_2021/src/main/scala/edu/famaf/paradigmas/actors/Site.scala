...
object Site {
  ...
  final case class SiteSubscription(url: String, urlParams: Seq[String], replyTo: ActorRef[SiteResponse]) extends SiteRequest
  ...
  override def onMessage(msg: SiteRequest): Behavior[SiteRequest] = {
    msg match{
      ...
      case FeedFailedMessage(msg) => 
        ...
        Behaviors.same
      case subMsg @ SiteSubscription(url, params, replyTo) =>
				val feedsAmount: Int = if (params.length == 0) {1} else {params.length}
        servicioFeed.subscribe(url, params, new RssSource())
        ...
        for (subs <- servicioFeed.subsList) {
          feeds.get(subs.url) match {
            ...
            case None =>
              ...
              val feedActor = context.spawn(Feed(),
                s"new-Feed-${context.self.path.name}-${feeds.size}")
