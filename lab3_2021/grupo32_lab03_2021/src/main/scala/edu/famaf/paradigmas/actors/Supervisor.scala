...
object Supervisor {
  ...
  final case class Subscribe(url: String, params: Seq[String]) extends SupervisorCommand
  ...
  override def onMessage(msg: SupervisorCommand): Behavior[SupervisorCommand] = {
    msg match {
      ...
      case StopWithMessage(msg) =>
        ...
        context.self ! Stop()
        Behaviors.same
      case subMsg @ Subscribe(enlace, parametros) =>
        val site = (siteRegex findAllIn enlace).mkString
        sites.get(site) match {
          case Some(siteActor) =>
            context.ask(siteActor, Site.SiteSubscription(enlace, parametros, _)) {
