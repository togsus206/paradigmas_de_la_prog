...
import datas.{RedditSource, RssSource, FeedServices}
...
object SubscriptionApp extends App {
  ...
  OParser.parse(argsParser, args, Config()) match {
    case Some(config) =>
      ...
      //
      system ! Supervisor.SetCounter(subscriptions.length)
      for (sub <- subscriptions){
        val parser = sub.urlType match{
          case "rss"    =>  new RssSource()
          case "reddit" =>  new RedditSource()
        }
        system ! Supervisor.Subscribe(sub.url, sub.feeds, parser)
      }
      //
